import numpy as np
from RealSamplingTools import *
from RBM_1dHeisenberg import *
from Hamiltonian_1dHeisenberg import *
...
class VMC:
    def __init__(
        ...
        J = 1, # AF Heisenberg interaction for nearest neighbor
#        hz = 0,
#        hx = 0,
        ...
        self.v_chain = None
        self.chainsamples = RealChainSamples()
        self.rbm = None
        self.eta = eta
        self.l2reg = l2reg
        self.sigma = sigma
...
#        self.hx = hx
        ...
        self.gsvec_exact = gsvec_exact
        self.overlaps = []

    # Initialize RBM
    def SetRBM(self):
        self.rbm = RealRBM(n_vis = self.n_vis, alpha = self.alpha, sigma = self.sigma, Delta = self.Delta, J = self.J,
...
#        Smat = np.diag(np.ones(self.n_param))
    ...
    def Der_all_vec_mean(self,v_chain):
        Der_all_vec_chain = np.zeros((self.n_batch, self.n_param))
    ...
    def G_means(self, v_chain):
        ...
        Der_Wf_chain = np.zeros((self.n_batch, self.n_vis, self.alpha))
        Der_a_chain = np.zeros((self.n_batch, self.n_vis))
        Der_bf_chain = np.zeros((self.n_batch, self.alpha))
        ...
        GWf_chain = np.array([  2 * np.real((Eloc_chain[j] - Eloc_mean) * np.conjugate(Der_Wf_chain[j]) )
                        for j in range(self.n_batch)])
        Ga_chain = np.array([  2 * np.real( (Eloc_chain[j] - Eloc_mean) * np.conjugate(Der_a_chain[j]) )
                            for j in range(self.n_batch)])
        Gbf_chain = np.array([  2 * np.real((Eloc_chain[j] - Eloc_mean) * np.conjugate(Der_bf_chain[j]) )
                            for j in range(self.n_batch)])
    ...
    def Update(self):
        ...
        if self.n_vis <=12:
            self.overlaps.append(self.GSoverlap())
    ...
    def UpdateParams(self, grads, params):
        ...
        if max(np.abs(grads)) > self.threshold2:
            grads = 0
    ...
    def GSoverlap(self):
        assert (self.gsvec_exact != None).any(), "Prepare the exact gs wavefunction to compare with ansatz."
        gsvec_raw = self.rbm.GSvec_raw()
        gsvec_exact = self.gsvec_exact
        return self.Overlap(gsvec_raw, gsvec_exact)
