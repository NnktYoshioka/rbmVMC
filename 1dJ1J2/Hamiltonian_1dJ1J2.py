...
def J1J2Ham(Leng, PBCflag = 0, J1 = 1, J2 = 1, Delta1 = 1, Delta2 = 1, hz = 0, hx = 0):
    """
...
#    hzranham =  np.sum([hranz[i] * Skron(3,i,Leng) for i in range(Leng)],axis = 0)
    ...
    # S_i^x -> - S_i^x,
    # S_i^y -> - S_i^y.
    J1Ham = 4 * J1 * np.sum([ + Skron(1,i,Leng).dot(Skron(1,i+1,Leng))
                            + Skron(2,i,Leng).dot(Skron(2,i+1,Leng))
                            + Delta1 * Skron(3,i,Leng).dot(Skron(3,i+1,Leng))
                            for i in range(Leng-1)], axis = 0) # 4 due to sigma(mu) = 2 * Skron

    J2Ham = 4 * J2 * np.sum([ Skron(1,i,Leng).dot(Skron(1,i+2,Leng))
                             + Skron(2,i,Leng).dot(Skron(2,i+2,Leng))
                            + Delta2 * Skron(3,i,Leng).dot(Skron(3,i+2,Leng))
                            for i in range(Leng-2)], axis = 0) # 4 due to sigma(mu) = 2 * Skron

    if PBCflag == 1:
        J1Ham = J1Ham + 4 * J1 *( Skron(1,0,Leng).dot(Skron(1,Leng-1,Leng))
                              +Skron(2,0,Leng).dot(Skron(2,Leng-1,Leng))
                            + Delta1 * Skron(3,0,Leng).dot(Skron(3,Leng-1,Leng))) # 4 due to sigma(mu) = 2 * Skron

        J2Ham = J2Ham + 4 * J2 *(Skron(1,0,Leng).dot(Skron(1,Leng-2,Leng))
                                + Skron(2,0,Leng).dot(Skron(2,Leng-2,Leng))
                                + Delta2 * Skron(3,0,Leng).dot(Skron(3,Leng-2,Leng))
                            + Skron(1,1,Leng).dot(Skron(1,Leng-1,Leng))
                                + Skron(2,1,Leng).dot(Skron(2,Leng-1,Leng))
                                + Delta2 * Skron(3,1,Leng).dot(Skron(3,Leng-1,Leng))) # 4 due to sigma(mu) = 2 * Skron


#    return hxranham + hyranham + hzranham + HeisenHam
    return J1Ham + J2Ham + hzham + hxham
